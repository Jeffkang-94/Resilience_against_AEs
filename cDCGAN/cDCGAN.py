...
class Generator(nn.Module):
    def __init__(self, params, label_num):
        super().__init__()

        # Input is the latent vector Z with label 
        self.tconv1 = nn.ConvTranspose2d(params['nz'], params['ngf']*4,
            kernel_size=4, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(params['ngf']*4)

        self.tconv1_label = nn.ConvTranspose2d(label_num, params['ngf']*4,
            kernel_size=4, stride=1, padding=0, bias=False)
        self.bn1_label = nn.BatchNorm2d(params['ngf']*4)
    ...
    def forward(self, x,y):
        ...
        y = F.relu(self.bn1_label(self.tconv1_label(y)))
        x = torch.cat([x,y],1)
...
class Discriminator(nn.Module):
    def __init__(self, params):
        ...
        self.conv1 = nn.Conv2d(params['nc'], params['ndf']/2,
            4, 2, 1, bias=False)
        self.conv1_label = nn.Conv2d(params['nc'], params['ndf']/2,
            4, 2, 1, bias=False)
    ...
    def forward(self, x, y):
        ...
        y = F.leaky_relu(self.conv1_label(y), 0.2, True)
        x = torch.cat([x,y],1)
